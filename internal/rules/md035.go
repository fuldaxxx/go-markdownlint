...
func init() { register(&md035) }

var md035 = rule.Rule{
	Names:       []string{"MD035", "hr-style"},
	Description: "Horizontal rule style",
	Tags:        []string{"hr"},
	Parser:      types.ParserMicromark,
	Fn: func(p *rule.RuleParams, onError types.OnError) {
		c := p.Config.MD035
		style := strings.TrimSpace(types.StringOr(c.Style, "consistent"))

		for _, token := range p.FilterByTypesCached([]mm.TokenType{mm.TypeThematicBreak}, false) {
			text := token.Text
			if style == "consistent" {
				style = text
			}

			helpers.AddErrorDetailIf(onError, token.StartLine, style, text, "", "", nil, nil)
		}
	},
}
