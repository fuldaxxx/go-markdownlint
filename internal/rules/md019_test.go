...
func TestMD019(t *testing.T) {
	t.Run("positive_multiple_space_atx", func(t *testing.T) {
		errs := lintRule(t, "MD019", "#  Heading\n")
		if !ruleFired(errs, "MD019") {
			t.Fatalf("expected MD019 to fire, got %v", errs)
		}
	...
	t.Run("negative_single_space_atx", func(t *testing.T) {
		errs := lintRule(t, "MD019", "# Heading\n")
		if ruleFired(errs, "MD019") {
			t.Fatalf("expected no MD019, got %v", errs)
		}
	})
}
